...
function setButtonsTitle() {
    ...
    tippy(msgerVideoUrlBtn, {
        content: 'Chia sẻ video YouTube cho tất cả những người tham gia',
    });
}
...
function whoAreYou() {
    ...
    Swal.fire({
        ...
        inputValidator: (value) => {
            if (!value) return 'xin hãy nhập tên của bạn';
    ...
    tippy(initAudioBtn, {
        content: 'Bấm vào để tắt âm thanh',
        placement: 'top',
    });
    tippy(initVideoBtn, {
        content: 'Bấm vào để tắt Video',
        placement: 'top',
    });
}
...
function welcomeUser() {
    ...
    Swal.fire({
        background: swalBackground,
        position: 'center',
        title: '<strong>Chào mừng bạn ' + myPeerName + '</strong>',
        ...
        <br/> 
        <p style="color:white;">
        Chia sẻ cuộc họp này mời những người khác tham gia.</p>
        <p style="color:rgb(8, 189, 89);">` +
            myRoomUrl +
            `</p>`,
        showDenyButton: true,
        showCancelButton: true,
        confirmButtonText: `Sao chép URL cuộc họp`,
        denyButtonText: `Thư mời qua email`,
        ...
        if (result.isConfirmed) {
            ...
            let message = {
                email: '',
                subject: 'Vui lòng tham gia Cuộc họp trò chuyện video VKU - Meeting của chúng tôi',
...
function handleVideoPlayerFs(videoId, videoFullScreenBtnId, peer_id = null) {
    ...
    function showMsg() {
        userLog('toast', 'Chế độ toàn màn hình hoạt động khi bật video');
    }
...
function setChatRoomBtn() {
    ...
    msgerCPBtn.addEventListener('click', (e) => {
        if (!thereIsPeerConnections()) {
            userLog('info', 'Không có người tham gia nào được phát hiện');
            return;
        }
    ...
    msgerSaveBtn.addEventListener('click', (e) => {
        ...
        userLog('info', 'Không có tin nhắn trò chuyện nào để lưu');
    });
...
async function shareRoomUrl() {
    ...
    if (!isSupportedNavigatorShare || (isSupportedNavigatorShare && errorNavigatorShare)) {
        ...
        Swal.fire({
            ...
            <br/><br/>
            <p style="color:white;"> Chia sẻ cuộc họp này mời những người khác tham gia.</p>
            <p style="color:rgb(8, 189, 89);">` +
                myRoomUrl +
                `</p>`,
            showDenyButton: true,
...
function copyRoomURL() {
    ...
    userLog('toast', 'URL cuộc họp được sao chép vào khay nhớ tạm  👍');
}
...
function toggleScreenSharing() {
    ...
    screenMediaPromise
        ...
        .catch((err) => {
            console.error('[Error] Không thể chia sẻ màn hình', err);
            userLog('error', 'Không thể chia sẻ màn hình ' + err);
        });
}
...
function notifyRecording(from, action) {
    let msg = '[ 🔴 GHI ] : ' + action + ' ghi lại màn hình và âm thanh của chính mình';
    let chatMessage = {
        from: from,
        to: myPeerName,
        msg: msg,
...
function sendChatMessage() {
    if (!thereIsPeerConnections()) {
        userLog('info', "Không thể gửi tin nhắn, không có người tham gia trong phòng");
        msgerInput.value = '';
        return;
    }
...
function addMsgerPrivateBtn(msgerPrivateBtn, msgerPrivateMsgInput) {
    ...
    msgerPrivateBtn.addEventListener('click', (e) => {
        ...
        appendMessage(myPeerName, rightChatAvatar, 'right', pMsg + '<br/><hr>Tin nhắn cá nhân tới ' + toPeerName, true);
...
//     if (status) {
//         userLog('toast', peer_name + ' đã giơ tay');
//         playSound('raiseHand');
//     }
// }
...
function emitRoomStatus() {
    let rStatus = roomLocked ? '🔒 ĐÃ KHÓA phòng, không ai có thể vào!' : '🔓 MỞ KHÓA phòng';
...
function handleWhiteboard(config) {
    ...
    switch (act) {
        case 'draw':
            ...
            break;
        case 'clean':
            userLog('toast', peer_name + ' đã lau bảng');
            whiteboardClean();
            break;
        case 'open':
            userLog('toast', peer_name + ' đã mở bảng');
            whiteboardOpen();
            break;
        case 'close':
            userLog('toast', peer_name + ' đã đóng bảng');
            whiteboardClose();
            break;
        case 'resize':
            userLog('toast', peer_name + ' đã thay đổi kích thước bảng');
            whiteboardResize();
            break;
    }
}
...
function sendFileData() {
    ...
    fileReader.addEventListener('load', (e) => {
        ...
        if (offset === fileToSend.size) {
            ...
            userLog('success', 'Tệp Tin ' + fileToSend.name + ' đã được gửi thành công.');
        }
...
function sendVideoUrl(peer_id = null) {
    ...
    Swal.fire({
        ...
        position: 'center',
        imageUrl: youtubeImg,
        title: 'Chia sẻ video trên YouTube',
        text: 'Điền URL video YouTube ',
        input: 'text',
        showCancelButton: true,
...
function openVideoUrlPlayer(config) {
    ...
    if (!isVideoUrlPlayerOpen) {
        if (videoEmbed) {
            ...
            userLog('error', 'Đã xảy ra lỗi, hãy thử với một URL Youtube khác');
        }
    } else {
...
function handleVideoPlayer(config) {
    ...
    switch (video_action) {
        case 'open':
            userLog('toast', peer_name + ' đã mở trình phát video');
            openVideoUrlPlayer(config);
            break;
        case 'close':
            userLog('toast', peer_name + ' đã đóng trình phát video');
            closeVideoUrlPlayer();
            break;
    }
}
...
function showAbout() {
    ...
    Swal.fire({
        ...
        <div id="about">
            ...
            <a href="https://www.facebook.com/cuonggdev/" target="_blank"><br/></br>
            
            <button class="far fa-heart pulsate" onclick="window.open('https://www.facebook.com/cuonggdev/')"> Sponsor</button>
            <br /><br />
            Author:<a href="https://www.linkedin.com/in/miroslav-pejic-976a07101/" target="_blank">Ngoc Cuong</a>
        </div>
        `,
        showClass: {
            popup: 'animate__animated animate__fadeInDown',
